...
class Counter(val numberToCount:Int=0){
  ...
  def inc:Counter={ new Counter(numberToCount+1)}
  def dec:Counter={new Counter(numberToCount-1)} // immutability
  def inc(offset:Int):Counter={new Counter(this.numberToCount+offset)}
  def dec(offset:Int):Counter={new Counter((this.numberToCount)-offset)}
